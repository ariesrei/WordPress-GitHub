...
*/
	...
	public function delete_transients($thesis = false) {
		if (!empty($thesis))
			delete_transient(apply_filters('thesis_transient', false));
		delete_transient('thesis_component_callout');
		delete_transient('thesis_callout');									// Legacy cleanup
	...
	public function activate($key = false, $product = false, $name = false, $class = false, $url = false, $response_key = false) {
		...
		$response = $this->post($thesis->store, array(
			...
			if ($license_data->success === false) {
				switch ($license_data->error) {
					...
					case 'invalid_item_id' :
						$message = sprintf(__('The license key you entered must match the key for %s from your My Account page at DIYthemes.', 'thesis'), $license_data->item_name);
						break;
					case 'item_name_mismatch' :
						$message = sprintf(__('This appears to be an invalid license key for %s.', 'thesis'), $license_data->item_name);
						break;
					case 'no_activations_left' :
						$message = __('Your license key has reached its activation limit.', 'thesis');
						break;
					default :
						$message = __('An unknown error occurred, please enter your license key again.', 'thesis');
						break;
				}
				if (!empty($message)) {
					wp_redirect(add_query_arg(array(
	...
	public function check_license($key = false, $product = false, $name = false, $class = false, $url = false, $response_key = false) {
		...
		if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
			$message = is_wp_error($response) ?
				$response->get_error_message() :
				"<span><strong>Status:</strong> ". __('License system failure, try again', 'thesis'). "</span>";
			wp_redirect(add_query_arg(array(
				'sl_theme_activation' => 'false',
				'msg' => urlencode($message)),
				admin_url($url)));
			...
			if (!isset($license_data->license))
				return "<span><strong>Status:</strong> ". __('Key failure, try again', 'thesis'). "</span>";
			if ($license_data && isset($license_data->license))
				update_option("{$class}_license_key_status", $license_data->license);
			// Get expire date
			...
			elseif ($license_data->license == 'site_inactive')
				...
				$message = "<span><strong>Status:</strong> ". __('Key failure, try again', 'thesis'). "</span>";
		}
		return $message;
	}
