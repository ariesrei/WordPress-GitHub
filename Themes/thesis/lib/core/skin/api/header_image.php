...
class thesis_skin_header_image {
	...
	*/
	public function admin() {
		...
		$docs = !empty($thesis->skin) && !empty($thesis->skin->docs) && !empty($thesis->skin->docs['header_image']) ?
			' <a data-style="dashicon" href="'. esc_url($thesis->skin->docs['header_image']). '" title="'. __('See Documentation', 'thesis'). '" target="_blank" rel="noopener">&#xf348;</a>' : '';
		echo
			"\t\t<h3>", sprintf(__('%1$sSkin Header Image', 'thesis'), $this->name), "$docs</h3>\n",
	...
	*/
	public function html() {
		...
		$ref = esc_attr(apply_filters('thesis_header_image', 'thesis_header_image'));
		$link = apply_filters('thesis_header_image_link', home_url());
		echo (!empty($link) ?
			"<a id=\"{$ref}_link\" href=\"". esc_url($link). "\">" : ''),
			"<img id=\"$ref\" src=\"", esc_url($thesis->api->url_current($this->image['src'])), "\" alt=\"", trim($thesis->api->ef((!empty($thesis->api->options['blogname']) ?
			htmlspecialchars_decode($thesis->api->options['blogname'], ENT_QUOTES). ' ' : ''). __('header image', 'thesis'))), "\" width=\"{$this->image['width']}\" height=\"{$this->image['height']}\" />",
			(!empty($link) ?
			"</a>" : ''), "\n";
	}
}