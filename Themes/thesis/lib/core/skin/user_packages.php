...
class thesis_user_packages {
	...
	public static function item_info($package, $active = array(), $updates = array(), $depth = 0) {
		...
		return
			...
			"<a onclick=\"if(!thesis_update_packages()) return false;\" href=\"". wp_nonce_url(admin_url("update.php?action=thesis_update_objects&type=package&class=$id&name=". urlencode($thesis->api->efh($package['name']))), 'thesis-update-objects'). "\">". sprintf(__('Update %s', 'thesis'), $thesis->api->efh($package['name'])). "</a>" : ''). "</label>\n".
