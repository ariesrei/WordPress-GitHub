...
thesis_colors = {
	init: function() {
		...
		$('.control_swatch').on('click', function() {
			...
			$(this).siblings('.'+$(this).data('value')).each(function() {
				var id = $(this).data('scheme')+'-'+$(this).data('id');
		...
		$('.complement').on('click', function() {
			...
			if ($(complements).hasClass('triggered'))
				thesis_colors.get_complements(complements, $(this).data('id'), $(this).siblings('.scheme').val());
		});
	},
