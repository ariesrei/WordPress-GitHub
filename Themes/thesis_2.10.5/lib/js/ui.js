...
thesis_ui = {
	...
	box: {
		init: function(box) {
			$(box).prepend('<input type=\"hidden\" class=\"box_location\" name=\"boxes['+$(box).parent().parent('.rotator').data('id')+'][]\" value=\"'+$(box).data('id')+'\" />');
		},
		options: function(trigger) {
			$(trigger).click(function() {
				thesis_ui.popup('#popup_'+$(this).parent('h4').parent().data('id'));
				return false;
			});
		},
		rotator: function(rotator) {
			...
			if ($(toggle).hasClass('toggled') || $(rotator).data('root') || $(rotator).attr('id') == 'delete_boxes') {
		...
		tray: function(tray) {
			...
			$(tray).children('h5').droppable({
				accept: '.parent_' + $(tray).parent('.rotator').data('id'),
				disabled: true,
		...
		dependent: function(dependent) {
			$(dependent).on('click', function() {
				$(this).hide(200, function() {
					...
					prepend('<input type=\"hidden\" class=\"box_location\" name=\"boxes['+$(this).parent('.tray_list').parent('.tray_body').parent('.tray').parent('.rotator').data('id')+'][]\" value=\"'+$(this).data('id')+'\" />').
					appendTo(destination).show('fast').unbind();
					$('#popup_'+$(this).data('id')).appendTo(destination);
				});
				return false;
			});
		},
	...
	popup: function(popup) {
		...
		$(body).find('.pane_'+$(menu).children('li:first').data('pane')).show();
		$(menu).children('li').on('click', function() {
			$(this).siblings().removeClass('active');
			$(this).addClass('active');
			$(body).find('.pane').hide();
			$(body).find('.pane_'+$(this).data('pane')).show();
		});
		$(popup+' .t_popup_name').on('keyup', function() { $('#'+$(this).data('id')).html($(this).val()); });
