...
class thesis_terms {
	...
	public function __construct() {
		$this->terms = is_array($terms = get_option('thesis_terms')) ? $terms : $this->terms;
		if (!is_admin()) return;
	...
	public function init_options() {
		...
		if (!($pagenow == 'edit-tags.php' || $pagenow == 'term.php') && $taxonomy != 'link_category') return;
				...
				foreach ($options as $id => $option)
					if (is_array($option)) {
						...
						if ($option['type'] == 'text' || $option['type'] == 'textarea') {
							...
							$description = !empty($option['description']) ?
								"<p class=\"description\">{$option['description']}</p>" : false;
							$value = !empty($values[$class][$id]) ? $values[$class][$id] : (!empty($option['default']) ? $option['default'] : '');
							$input = $option['type'] == 'text' ?
	...
	public function create_term($term_id) {
		...
		foreach ($this->options as $class => $options)
			if (is_array($options)) {
				...
				if ($new[$class] = $thesis->api->set_options($options, stripslashes_deep($_POST[$class])))
