...
/*---:[ Initialize core Skin functionality through actions and filters ]:---*/
	...
	private function _actions() {
		...
		if (method_exists($this, 'front_end'))
		...
		if ((!$thesis->environment && !is_admin()) || $thesis->environment == 'canvas') {
			...
			if (!$thesis->environment && apply_filters('thesis-editor-launcher', true)) {
	...
	private function _filters() {
		...
		if (!is_admin()) return;
		$thesis->box_post_meta[] = 'thesis_redirect';
		$thesis->box_post_meta[] = 'thesis_skin';
	...
	public function _init_admin() {
		...
		if (!empty($this->_instances) || method_exists($this, 'display'))
			$this->_menu["{$this->_class}__content"] = array(
				...
				'description' => __('Manage what displays in your Skin and edit selected content', 'thesis'));
		...
		if (method_exists($this, 'design') || method_exists($this, 'design_admin') || !empty($this->functionality['design_admin'])) {
			$this->_menu["{$this->_class}__design"] = array(
				...
				'description' => __('Change certain aspects of your design, like fonts and colors', 'thesis'));
...
*/
	public function _admin_home() {
		...
		echo
			"\t\t\t<h3>", trim($thesis->api->efh(sprintf(__('%s Options', 'thesis'), $this->_name))), "</h3>\n",
...
/*---:[ Skin License Key Page ]:---*/
	...
	public function _license_key_admin() {
		...
		$message = !empty($_GET['msg']) ? urldecode($_GET['msg']) : get_transient("{$this->_class}_license_message", false);
...
/*---:[ Thesis Skin Editor ]:---*/
	...
	public function _append_menu_links($menu) {
		$skin['editor'] = array(
			...
			'icon' => '&#59190;',
			'description' => __('Edit templates and also manage Skin data (advanced)', 'thesis'));
		if (!empty($this->_docs))
			$skin['docs'] = array(
				'text' => sprintf(__('%s Docs', 'thesis'), $this->_name),
				'url' => esc_url($this->_docs),
				'description' => __('Explore this Skin&#8217;s documentation', 'thesis'));
		if (!empty($this->license_key))
	...
	public function _editor_scripts() {
		global $thesis;
		$scripts = array();
		$scripts['jquery'] = 'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.min.js';
		$scripts['jquery-ui'] = 'https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js';
...
/*---:[ Thesis HTML Head Editor (separate from the Skin Editor; appears in the Thesis Admin) ]:---*/
	...
	public function _admin_init_head_editor() {
		global $thesis;
		wp_enqueue_style('thesis-options');
		wp_enqueue_style('thesis-box-form');
		wp_enqueue_script('thesis-jquery',
			'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.min.js', array(), $thesis->version, true);
		wp_enqueue_script('thesis-jquery-ui',
			'https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js', array('thesis-jquery'), $thesis->version, true);
		wp_enqueue_script('thesis-options');
		wp_enqueue_script('thesis-ui',
			THESIS_JS_URL. '/ui.js', array('thesis-menu', 'thesis-options'), $thesis->version, true);
...
/*---:[ Front-end Skin output ]:---*/
	...
	public function _editor_launcher_css() {
		...
		echo
			"<style>\n",
			"#thesis_launcher { position: fixed; bottom: 0; $position: 0; font: bold 16px/1em system-ui, BlinkMacSystemFont, Roboto, \"Segoe UI\", Segoe, \"Helvetica Neue\", Tahoma, sans-serif; padding: 12px; text-align: center; color: #fff; background: rgba(0,0,0,0.5); text-shadow: 0 1px 1px rgba(0,0,0,0.75); }\n",
	...
	public function _post_meta($post_meta) {
		...
		$options = array(
			'thesis_redirect' => array(
				...
				'cpt' => apply_filters('thesis_redirect_post_meta_cpt', array(
					'post',
					'page')),
				'fields' => array(
					'url' => array(
			...
			$this->_class => array(
				...
				'cpt' => apply_filters('thesis_skin_post_meta_cpt', array(
					'post',
					'page')),
				'context' => 'side',
				'priority' => 'default',
