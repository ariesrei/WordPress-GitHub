...
/*		Note: Per page setting has been disabled per WP's f-up with version 4.4.
	...
	public function preload() {
		global $thesis;
		if (apply_filters("{$this->_class}_preload", true))
...
class thesis_comment_date extends thesis_box {
	...
	protected function html_options() {
		...
		$html['class']['tooltip'] = sprintf(__('This box already contains a %1$s of <code>comment_date</code>. If you&#8217;d like to supply another %1$s, you can do that here.%2$s', 'thesis'), $thesis->api->base['class'], __($thesis->api->strings['class_note'], 'thesis'));
		unset($html['id']);
		return array_merge($html, array(
			'format' => array(
				'type' => 'text',
