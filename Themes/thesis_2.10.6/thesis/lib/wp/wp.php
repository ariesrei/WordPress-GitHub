...
class thesis_wp {
	...
	private function actions() {
		...
		add_action('hook_before_html', array($this, 'wp_body_open'), 1);
	...
	public function init_post_meta() {
		...
		foreach ($this->post_meta as $class => $meta) {
			new thesis_post_meta($class, apply_filters("{$class}_post_meta", $meta), $tabindex);
...
*/
	...
	public function add_widget_area($id, $name, $description = false) {
		if (empty($id) || empty($name)) return;
		register_sidebar(array(
			'id' => $id,
			'name' => $name,
			'description' => $description,
			'before_widget' => "<div class=\"widget %2\$s". (!empty($this->options['class']) ? ' '. trim(esc_attr($this->options['class'])) : ''). '" id="%1$s">',
			'after_widget' => "</div>",
