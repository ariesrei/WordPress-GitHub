...
class thesis_form_api {
			...
			if (!empty($field['type'])) {
				...
				$form_field = $field['type'] == 'group' ?
					$this->field_group(
						...
						$name_prefix,
						$tabindex,
						$depth) : ($field['type'] == 'object_set' ?
					$this->field_object_set(
						$field,
						$values,
						$id_prefix,
						$name_prefix,
						$tabindex,
						$depth) : ($field['type'] == 'custom' && !empty($field['output']) ?
					$this->field_custom(
						$field,
						$tabindex,
						$depth) :
	...
	public function field_group($field, $values = array(), $id_prefix = false, $name_prefix = false, $tabindex = 1, $depth = 0) {
		...
		$id = !empty($field['item_id']) ? ' id="'. esc_attr($field['item_id']). '"' : '';
		...
		$group = $this->fields(
			...
			$name_prefix,
			$tabindex,
			!empty($field['label']) ? $depth + 2 : $depth + 1);
		$group['output'] =
			"$tab<div$id class=\"$class\">\n".
			(!empty($field['label']) ?
	...
	public function field_object_set($field, $values = array(), $id_prefix = false, $name_prefix = false, $tabindex = 1, $depth = 0) {
		...
		foreach ($field['objects'] as $id => $object)
			if (is_array($object) && is_array($object['fields'])) {
				$objects[$id] = $this->object(
					array_merge($object, array('id' => $id)),
	...
	public function object($object, $values = array(), $id_prefix = false, $name_prefix = false, $tabindex = 1, $depth = 0) {
		...
		$fields = $this->fields(
			$object['fields'],
			$values,
			"$id_prefix{$object['id']}_",
			"{$name_prefix}[{$object['id']}]",
			$tabindex,
			$depth);
	...
	public function field_custom($field, $tabindex = 1, $depth = 0) {
	...
	public function field($field, $id, $name, $value = false, $tabindex = 1, $depth = 0) {
		...
		$item_id = !empty($field['item_id']) ? ' id="'. esc_attr($field['item_id']). '"' : '';
