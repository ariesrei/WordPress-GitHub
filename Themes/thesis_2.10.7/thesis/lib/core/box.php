...
class thesis_box {
	...
	public function __construct($box = array()) {
		...
		// Taxonomies appear before Post Types because WP is a clown show
		if (method_exists($this, 'post_type_taxonomies'))
			$this->_post_type_taxonomies();
		if (method_exists($this, 'post_types'))
			$this->_post_types();
	...
	private function _post_type_taxonomies() {
		$taxonomies = $this->post_type_taxonomies();
		if (is_array($taxonomies))
			foreach ($taxonomies as $name => $tax)
				if (!empty($tax['cpt']) && !empty($tax['taxonomy']))
					register_taxonomy(
						apply_filters("$name-key", $name),
						apply_filters("$name-cpt", $tax['cpt']),
						apply_filters("$name-args", $tax['taxonomy']));
	}
	...
	private function _post_types() {
		...
		if (is_array($types))
			foreach ($types as $type => $cpt)
				if (apply_filters("$type-cpt", true))
					register_post_type(
						apply_filters("$type-key", $type),
						apply_filters("$type-args", $cpt));
	}

	public function _add_post_meta($post_meta) {
		return
			is_array($options[$this->_class] = $this->post_meta())
			&& $this->_display() ?
				(is_array($post_meta) ?
	...
	private function _class_admin() {
		...
		add_filter(!empty($this->filters['menu']) ? ($this->filters['menu'] == 'site' ?
			...
			'thesis_skin_menu' : ($this->filters['menu'] == 'custom' ? false : 'thesis_boxes_menu'))) :
