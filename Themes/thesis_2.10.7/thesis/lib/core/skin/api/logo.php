...
*/
	public function __construct() {
		...
		if (!empty($_GET['canvas']) && $_GET['canvas'] == "{$this->skin}__logo") {
...
*/
	public function menu($menu) {
		$menu["{$this->skin}_logo"] = array(
			'text' => __('Logo', 'thesis'),
			'url' => "admin.php?page=thesis&canvas={$this->skin}__logo",
...
*/
	public function save() {
		...
		wp_redirect(admin_url("admin.php?page=thesis&canvas={$this->skin}__logo"));
		exit;
	}
...
*/
	public function html($title) {
		...
		return empty($this->image) ? false :
			"<img id=\"$ref\" src=\"". esc_url($thesis->api->url_current($this->image['src'])). "\" alt=\"$title\" width=\"{$this->image['width']}\" height=\"{$this->image['height']}\" />";
	}
}