...
thesis_editor = {
	...
	init_backups: function() {
		$('.t_restore_backup').on('click', function() {
			thesis_editor.restore($(this).data('id'), confirm("Create an automatic backup before proceeding?\n\n(The cancel button will perform the restoration without creating a backup.)") ? 'true' : 'false');
		});
		$('.t_export_backup').on('click', function() {
			$('#t_export_id').val($(this).data('id'));
		...
		$('.t_delete_backup').on('click', function() {
			if (confirm('Are you sure you want to do this? Once you delete a backup, it cannot be recovered.'))
				thesis_editor.delete($(this).data('id'));
		});
	},
